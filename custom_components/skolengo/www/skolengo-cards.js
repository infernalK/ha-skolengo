...
(function () {
  ...
  const BASE_STYLE = `
    ...
    .skolengo-period-selector {
      display: flex;
      flex-wrap: wrap;
      gap: 6px;
      margin-bottom: 12px;
    }
    .skolengo-period-selector button {
      font: inherit;
      font-size: 0.85em;
      color: var(--primary-text-color);
    ...
    .skolengo-period-selector button.active {
  ...
  // module), since each card instance re-renders its own shadow DOM.
  function renderPeriodSelector(periods, selectedPeriodId) {
    if (!Array.isArray(periods) || !periods.length) return "";
    const chip = (id, label) =>
      `<button type="button" data-period-id="${escapeHtml(id)}" class="${
        selectedPeriodId === id ? "active" : ""
      }">${escapeHtml(label)}</button>`;
    return `<div class="skolengo-period-selector">
      ${chip("", "Total")}
      ${periods.map((p) => chip(p.id, p.label || "?")).join("")}
    </div>`;
  }

  // Wires clicks on a rendered period selector to `onSelect(periodId)`
  // ("" means "Total" / all periods combined).
  function bindPeriodSelector(root, onSelect) {
    const selector = root.querySelector(".skolengo-period-selector");
    if (!selector) return;
    selector.addEventListener("click", (event) => {
      const btn = event.target.closest("button[data-period-id]");
      if (!btn) return;
      onSelect(btn.dataset.periodId || "");
    });
  }

  // Picks the default selected period: the one flagged `current`, else
  // none ("Total").
  ...
  class SkolengoEvaluationsCard extends HTMLElement {
    ...
    _render() {
      ...
      let html = "";
      if (this._config.display_header) {
        // The "Notes" sensor carries a ready-computed "average" attribute
        // (same value as the separate "Moyenne générale" sensor's state)
        // -- read it directly rather than recomputing it here.
        const avgLabel =
          typeof avg === "number" && !isNaN(avg) ? `${avg.toFixed(2)}/20` : "Moyenne indisponible";
        html += `<div class="skolengo-header">
  ...
  class SkolengoAveragesCard extends HTMLElement {
    ...
    _render() {
      ...
      const byPeriod = attrs.average_by_period || {};
      const periodData = this._selectedPeriodId ? byPeriod[this._selectedPeriodId] : null;
      const overall = periodData ? periodData.average : parseFloat(stateObj.state);
      const bySubject = (
        Array.isArray(periodData ? periodData.by_subject : attrs.by_subject)
          ? (periodData ? periodData.by_subject : attrs.by_subject).slice()
          : []
      ...
      if (this._config.display_header) {
        const overallLabel =
          typeof overall === "number" && !isNaN(overall) ? `${overall.toFixed(2)}/20` : "Moyenne indisponible";
        html += `<div class="skolengo-header">
          <span class="skolengo-title">${escapeHtml(this._config.title)}</span>
