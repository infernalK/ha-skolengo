...
- **Notes** : l'endpoint des évaluations/notes est connu pour être instable ou indisponible selon les établissements dans l'API Skolengo elle-même (pas seulement dans cette intégration). Le capteur de moyenne générale peut donc rester à `inconnu` pour votre établissement — ce n'est pas nécessairement un bug de l'intégration.
